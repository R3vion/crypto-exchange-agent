...
class QueryAnalysis(BaseModel):
    ...
    operation: Operation = Field(
        description=(
            "The main operation required to answer "
            "the user's question."
        )
    )
    ...
    exchanges: list[str] = Field(
        ...
        description=(
            "Cryptocurrency exchanges explicitly "
            "mentioned or clearly requested by the user."
        ),
    )
    ...
    jurisdiction: Jurisdiction = Field(
        ...
        description=(
            ...
            "Use exactly one of: EU, US, UK, GLOBAL, UNKNOWN."
        ),
    )

    requires_rag: bool = Field(
        description=(
            "Whether external documents or "
            "structured knowledge are required."
        ),
    )

    requires_calculation: bool = Field(
        description=(
            "Whether numerical calculation or "
